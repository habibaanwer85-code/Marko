...
const WEB_SEARCH_TOOL = {
  ...
  function: {
    name: "web_search",
    description:
      "Search the live web for current, external, or verifiable information (prices, competitors, market trends, news, statistics, current products, \"latest\"/\"current\"/\"now\" type questions). Prefer calling this whenever the question could have changed since your training data — do not answer time-sensitive or current-state questions from memory alone.",
    parameters: {
      type: "object",
      properties: {
...
async function callGroqWithSearch(messages, system, forceSearch) {
  ...
  if (hasTavily) {
    ...
    // forceSearch guarantees a real search happens (used by the Research
    // Agent flow, which is explicitly a web-research step, not a maybe).
    // Otherwise leave it to the model's own judgment ("auto") to avoid
    // wasting search credits on questions that don't need one.
    baseBody.tool_choice = forceSearch
  ...
  // Sanitize the assistant message before replaying it: Groq's gpt-oss models
  // return a "reasoning_content" field on the first response that the API
  // rejects if you send it straight back on the follow-up call. Keep only
  // the fields the API actually needs.
...
export default async function handler(req, res) {
  ...
  try {
    const { messages, system, forceSearch } = req.body || {};
